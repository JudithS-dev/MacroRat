...
void loop() {
  ...
  if(millis() - last_time > 500){
    noInterrupts();
    long current_ticks_left = encoder_ticks_left;
    long current_ticks_right = encoder_ticks_right;
    interrupts();
    
    float revolutions_left = (float)current_ticks_left / TICKS_PER_REV;
    float revolutions_right = (float)current_ticks_right / TICKS_PER_REV;

    
    Serial.printf("Ticks left: %ld | Umdr. left: %.3f | Ticks right: %ld | Umdr. right: %.3f\n", 
                  current_ticks_left, revolutions_left, current_ticks_right, revolutions_right);
    #if COM_IS_USED
    communicationInterface->log("Ticks left: %ld | Umdr. left: %.3f | Ticks right: %ld | Umdr. right: %.3f", 
                                current_ticks_left, revolutions_left, current_ticks_right, revolutions_right);
    #endif
...
void loop(){
  ...
  // Gyro-basierte Korrektur (P-Regler)
  const float KP_GYRO = 70.0f;  // Tuning nötig
  int correction = (int)(heading_error * KP_GYRO);

  // Motorgeschwindigkeiten anpassen
  int left_speed =  ((int) (speed * 0.80)) - correction;
